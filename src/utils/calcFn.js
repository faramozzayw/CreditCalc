...
export const amountOfPayment = (monthlyPayment = 0, term = 0) => monthlyPayment * term;
...
export const calcFn = {
	"payoutAmount": state => {
		const { monthlyPayment, loanTerm } = state;
		console.log("payoutAmount")

		const result = amountOfPayment(monthlyPayment, loanTerm);
